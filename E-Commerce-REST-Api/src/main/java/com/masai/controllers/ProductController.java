...
//	adding new product in database
	...
	public ResponseEntity<Product> addProductHandler(@PathVariable("adminkey") String key,@Valid @RequestBody Product product)
	...
	public ResponseEntity<Product> updateProductHandler(@Valid @RequestBody Product product, @PathVariable("adminkey") String key)
			throws ProductNotFoundException, AdminException {
		CurrentAdminSession loggedInAdmin = asDao.findByUuid(key);
...
//	getting all products details from database
	...
	public ResponseEntity<Product> viewProductByIdHandler(@PathVariable("id") Integer productId) throws ProductNotFoundException {
	...
	public ResponseEntity<Product> setProductQuantityByProductIdHandler(@PathVariable("adminkey")String key,@PathVariable("id") Integer productid,
			@PathVariable("quantity") Integer quantity) throws ProductNotFoundException, AdminException {
		CurrentAdminSession loggedInAdmin = asDao.findByUuid(key);
		if (loggedInAdmin == null) {
			throw new AdminException("Please provide a valid key");
		} else {
	...
	public ResponseEntity<Product> setProductPriceByProductIdHandler(@PathVariable("adminkey") String key,@PathVariable("id") Integer productid,
			@PathVariable("price") Double price) throws ProductNotFoundException, AdminException {
		CurrentAdminSession loggedInAdmin = asDao.findByUuid(key);
		if (loggedInAdmin == null) {
			throw new AdminException("Please provide a valid key");
		} else {
